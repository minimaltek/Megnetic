...
final class AudioEngine: ObservableObject {
    ...
    // Raw FFT magnitudes for BOX pattern (not @Published to avoid SwiftUI thrashing)
    ...
    @objc private func flushAudioUpdates(_ link: Any?) {
    ...
    func start() {
        ...
        inputNode.installTap(onBus: 0, bufferSize: bufferSize, format: format) { [weak self] buffer, time in
            ...
            Task { @MainActor [weak self] in
                ...
                // Store gain-scaled FFT magnitudes for BOX pattern
                if !result.magnitudes.isEmpty {
                    self.fftMagnitudes = result.magnitudes.map { $0 * gain }
                }
...
nonisolated final class FFTProcessor: @unchecked Sendable {
    ...
    struct AnalysisResult: Sendable {
        ...
        var magnitudes: [Float]      // raw FFT magnitudes (512 bins)
    }
    ...
    func process(buffer: AVAudioPCMBuffer, sensitivity: Float) -> AnalysisResult {
        guard let fftSetup = fftSetup,
            ...
            return AnalysisResult(bass: 0, mid: 0, high: 0, spectralFlux: 0, bassFlux: 0, midFlux: 0, highFlux: 0, zcr: 0, magnitudes: [])
        }
